              ...
              if ($sql = "SELECT * FROM carros WHERE estado='ocupado'") {
                ...
                if ($resultado->num_rows > 0) {
                  ...
                  while ($row = $resultado->fetch_assoc()) {
                    $ocupados = "<p class=''><button type='button'class='btn btn-danger' title='" .($row['fornecedor']) . "; " . ($row[ 'estado']). "; " .($row['matricula']) . "; " . ($row[ 'marca']).  "; " . ($row[ 'modelo']). "; " . ($row[ 'cor']). "'> <img class='p-2' src='icon/icon.png'><br>"  .$row['id'].  " </button><p></br>";
                    echo $ocupados;
